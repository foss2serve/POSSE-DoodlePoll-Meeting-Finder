...
def test_from_dicts():
    ...
    ms = list(meetings.from_dicts(ms))
    assert len(ms) == 2
    m = ms[0]
...
def test_from_tuples():
    ...
    ms = list(meetings.from_tuples(ms))
    assert len(ms) == 3
...
def test_filter_meetings_by_start_time():
    import operator
    ms = meetings.from_tuples([
        ('May 2019 Mon 20 12:00 AM', [], []),
        ('May 2019 Tue 21 1:00 AM', [], []),
        ('May 2019 Wed 22 2:00 PM', [], [])
    ])
    ms = list(meetings.filter_meetings(ms, 'start_hour_24', operator.ge, 1))
    assert len(ms) == 2
...
def test_filter_meetings_by_weekday():
    ...
    ms = list(
        meetings.filter_meetings(ms, 'weekday', operator.contains, (0, 2)))
    assert len(ms) == 2
